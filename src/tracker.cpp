...
void TrackerModule::onStopRecording()
{
  ...
  for (;;)
  {
    ...
    else if (row.length() >= 5)
    {
      ...
      //  segment.points.resize(segment.points.size() + 1000);
  ...
  if (c > 0)
  {
    ...
    gpx.name = osmscout::gpx::Optional<std::string>::of(file->fileName().toUtf8().toStdString());
    ...
    fdate.setSecsSinceEpoch(std::chrono::duration_cast<std::chrono::duration<double> >(beg.time.get().time_since_epoch()).count());
